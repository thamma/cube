...
import me.thamma.cube.interpreter.FuckOffPlsWork;
...
public class Cube {
    ...
    public String getFaceletDefinition() {
        String out = "";
        for (Sticker sticker: FuckOffPlsWork.faceletDefinition)
    ...
    public boolean equals(Object o) {
        ...
        for (int i = 0; i < FuckOffPlsWork.faceStickers.length; i++)
            for (int j = 0; j < FuckOffPlsWork.faceStickers[i].length; j++){
                if (local.getCurrentStickerAt(FuckOffPlsWork.faceStickers[i][j]) != ref.getCurrentStickerAt(FuckOffPlsWork.faceStickers[i][j]))
                    return false;}
        return true;
    }
}
